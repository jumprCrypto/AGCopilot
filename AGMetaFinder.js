(async function () {
    ...
    // Format large numbers (handles null/undefined)
                    ...
                    ">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 6px;">
                            <button onclick="window.applyArchetypeConfig(${index})" style="
                                ...
                                font-weight: 600;
                            ">⚙️ Apply</button>
                            <button onclick="window.setRequiredTokens(${index})" id="lock-btn-${index}" style="
                                padding: 6px;
    ...
    // Set required tokens from an archetype for optimizer constraint
    window.setRequiredTokens = function(index) {
        if (!currentArchetypes || !currentArchetypes.archetypes || !currentArchetypes.archetypes[index]) {
            console.error('No archetype data available');
            return;
        }
        ...
        // Store required tokens globally for the optimizer to use
        window.REQUIRED_TOKENS = {
        ...
        if (btn) {
            ...
            btn.style.background = 'rgba(72, 187, 120, 0.4)';
            btn.style.borderColor = 'rgba(72, 187, 120, 0.6)';
        }
        
        // Reset other lock buttons
        document.querySelectorAll('[id^="lock-btn-"]').forEach(b => {
            if (b.id !== `lock-btn-${index}`) {
                b.innerHTML = '🔒 Lock';
                b.style.background = 'rgba(72, 187, 120, 0.2)';
                b.style.borderColor = 'rgba(72, 187, 120, 0.4)';
            }
        });
        
        // Update required tokens indicator in AGCopilot UI
        const indicator = document.getElementById('required-tokens-indicator');
        const indicatorText = document.getElementById('required-tokens-text');
        if (indicator && indicatorText) {
            indicator.style.display = 'block';
            indicatorText.innerHTML = `🔒 Required: ${arch.name} (${tokenAddresses.length} tokens)`;
        }
        
        console.log(`🔒 Set ${tokenAddresses.length} required tokens from archetype #${index + 1}: ${arch.name}`);
        console.log(`   These tokens will be enforced during optimization scoring`);
        
        if (typeof window.updateStatus === 'function') {
            window.updateStatus(`🔒 Locked ${tokenAddresses.length} tokens from ${arch.name}`);
        }
    };
    
    // Clear required tokens constraint
    window.clearRequiredTokens = function() {
        window.REQUIRED_TOKENS = null;
        
        // Reset all lock buttons
        document.querySelectorAll('[id^="lock-btn-"]').forEach(btn => {
            btn.innerHTML = '🔒 Lock';
            btn.style.background = 'rgba(72, 187, 120, 0.2)';
            btn.style.borderColor = 'rgba(72, 187, 120, 0.4)';
        });
        
        // Hide required tokens indicator in AGCopilot UI
        const indicator = document.getElementById('required-tokens-indicator');
        if (indicator) {
            indicator.style.display = 'none';
        }
        
        console.log('🔓 Cleared required tokens constraint');
        
        if (typeof window.updateStatus === 'function') {
            window.updateStatus('🔓 Token constraint cleared');
        }
    };
